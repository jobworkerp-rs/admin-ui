...
import { JobResultId } from "./job_result";
...
/**
 * # For Item Failed Event
 * Per-iteration failure inside a parallel/sequential ForTask running with
 * `onError: continue`. The for-task itself still emits exactly one final
 * TaskCompleted; this event surfaces the per-item failure without claiming
 * the for-task is done. Carries the captured error/item payload as JSON so
 * gRPC clients can render or count individual failures without needing a
 * follow-up call.
 */
export interface ForItemFailedEvent {
  /**
   * # Task Name
   * Name of the for-task as defined in the workflow.
   */
  taskName: string;
  /**
   * # Position
   * Position in the workflow (JSON Pointer format) of the failing iteration.
   */
  position: string;
  /**
   * # Iteration Index
   * Zero-based index of the failing item within the iterable.
   */
  index: number;
  /**
   * # Error Payload (JSON-encoded)
   * Self-describing payload containing at least `error`, `index`, and `item`.
   * Encoded as a JSON string to avoid coupling to the workflow Error proto.
   */
...
export interface WorkflowEvent {
  ...
  /**
   * # For Item Failed
   * Emitted for each iteration of a ForTask that failed under
   * `onError: continue`. The ForTask itself still emits a single final
   * TaskCompleted alongside this stream of failure events.
   */
  forItemFailed?: ForItemFailedEvent | undefined;
}

function createBaseJobStartedEvent(): JobStartedEvent {
  return { jobId: undefined, runnerName: "", workerName: undefined, position: "" };
}
...
function createBaseJobCompletedEvent(): JobCompletedEvent {
  return { jobId: undefined, jobResultId: undefined, position: "" };
}
...
export const JobCompletedEvent: MessageFns<JobCompletedEvent> = {
  encode(message: JobCompletedEvent, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.jobResultId !== undefined) {
      JobResultId.encode(message.jobResultId, writer.uint32(18).fork()).join();
    }
    if (message.position !== "") {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2: {
          ...
          message.jobResultId = JobResultId.decode(reader, reader.uint32());
          continue;
        }
        case 3: {
          if (tag !== 26) {
  ...
  fromPartial(object: DeepPartial<JobCompletedEvent>): JobCompletedEvent {
    const message = createBaseJobCompletedEvent();
    message.jobId = (object.jobId !== undefined && object.jobId !== null) ? JobId.fromPartial(object.jobId) : undefined;
    message.jobResultId = (object.jobResultId !== undefined && object.jobResultId !== null)
      ? JobResultId.fromPartial(object.jobResultId)
      : undefined;
    message.position = object.position ?? "";
    return message;
  },
...
function createBaseForItemFailedEvent(): ForItemFailedEvent {
  return { taskName: "", position: "", index: 0, errorPayloadJson: "" };
}

export const ForItemFailedEvent: MessageFns<ForItemFailedEvent> = {
  encode(message: ForItemFailedEvent, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.taskName !== "") {
    ...
    if (message.errorPayloadJson !== "") {
      writer.uint32(34).string(message.errorPayloadJson);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ForItemFailedEvent {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseForItemFailedEvent();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
        ...
        case 4: {
          ...
          message.errorPayloadJson = reader.string();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  create(base?: DeepPartial<ForItemFailedEvent>): ForItemFailedEvent {
    return ForItemFailedEvent.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<ForItemFailedEvent>): ForItemFailedEvent {
    const message = createBaseForItemFailedEvent();
    message.taskName = object.taskName ?? "";
    message.position = object.position ?? "";
    message.index = object.index ?? 0;
    message.errorPayloadJson = object.errorPayloadJson ?? "";
    return message;
  },
};

function createBaseStreamingDataEvent(): StreamingDataEvent {
  return { jobId: undefined, data: new Uint8Array(0) };
}
...
export const WorkflowEvent: MessageFns<WorkflowEvent> = {
  encode(message: WorkflowEvent, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.forItemFailed !== undefined) {
      ForItemFailedEvent.encode(message.forItemFailed, writer.uint32(66).fork()).join();
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 8: {
          ...
          message.forItemFailed = ForItemFailedEvent.decode(reader, reader.uint32());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromPartial(object: DeepPartial<WorkflowEvent>): WorkflowEvent {
    ...
    message.streamingData = (object.streamingData !== undefined && object.streamingData !== null)
      ...
      : undefined;
    message.forItemFailed = (object.forItemFailed !== undefined && object.forItemFailed !== null)
      ? ForItemFailedEvent.fromPartial(object.forItemFailed)
      : undefined;
    return message;
  },
};
