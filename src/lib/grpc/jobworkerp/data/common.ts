...
export enum RunnerType {
  ...
  MCP_SERVER = 7,
  /**
   * FUNCTION_SET_SELECTOR - # FunctionSet Selector Runner
   * Lists available FunctionSets with tool summaries for LLM tool selection.
   * Used as a meta-tool to help LLM discover and select appropriate FunctionSets.
   */
  FUNCTION_SET_SELECTOR = 8,
  /**
   * LLM - # LLM Runner (Multi-method)
   * Unified LLM runner with 'completion' and 'chat' methods
   * - completion: Text completion using LLM (prompt-based)
   * - chat: Chat conversation with message history
   * Requires 'using' parameter to specify method
   */
  LLM = 32768,
  /**
   * WORKFLOW - # Workflow Runner (Multi-method)
   * Unified workflow runner with 'run' and 'create' methods
   * - run: Execute workflow with pre-configured definition (default)
  ...
  LLM_CHAT = 65533,
  /**
   * LLM_COMPLETION - # LLM Completion API Runner [DEPRECATED]
   * Deprecated: Use LLM runner with 'completion' method instead
   * Interacts with Large Language Models
   */
  LLM_COMPLETION = 65534,
  /**
   * INLINE_WORKFLOW - # Inline Workflow Runner [DEPRECATED]
   * Deprecated: Use temporary worker with WORKFLOW runner instead
  ...
  /**
   * REUSABLE_WORKFLOW - # Reusable Workflow Runner [DEPRECATED]
   * Deprecated: Use WORKFLOW runner with 'run' method instead
  ...
  /**
   * CREATE_WORKFLOW - # Create Workflow Runner [DEPRECATED]
   * Deprecated: Use WORKFLOW runner with 'create' method instead
...
export enum StreamingType {
   ...
   *
   * Key behavior differences from STREAMING_TYPE_RESPONSE:
   * - Even with ResponseType::Direct workers, enqueue returns immediately
   *   (does not wait for job completion) to allow stream subscription
   *   before data is published
   * - The caller is responsible for subscribing to the stream and collecting
   *   results using RunnerSpec::collect_stream()
   * - Final collected result is sent as ResultOutputItem::final_collected
   */
