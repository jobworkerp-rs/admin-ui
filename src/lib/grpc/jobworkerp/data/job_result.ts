...
import { Priority, ResponseType, ResultOutput, ResultStatus, RetryPolicy, StreamingType } from "./common";
...
export interface JobResultData {
  ...
  /**
   * # Result Output
   * Job execution output data (non-streaming). On success, items contains the
   * runner method's result protobuf. On failure, items contains a UTF-8
  ...
  using?:
    ...
    | undefined;
  /**
   * # Broadcast Results Flag
   * from worker.data.broadcast_results
   * Controls whether job results are published to broadcast channels
   */
  broadcastResults: boolean;
  /**
   * # Resolved Retry Policy
   * Retry policy resolved from worker defaults and per-job overrides
   * Used for retry decisions and periodic job re-enqueue
   */
  resolvedRetryPolicy?: RetryPolicy | undefined;
}
...
export const JobResultData: MessageFns<JobResultData> = {
  encode(message: JobResultData, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.broadcastResults !== false) {
      writer.uint32(176).bool(message.broadcastResults);
    }
    if (message.resolvedRetryPolicy !== undefined) {
      RetryPolicy.encode(message.resolvedRetryPolicy, writer.uint32(186).fork()).join();
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 23: {
          ...
          message.resolvedRetryPolicy = RetryPolicy.decode(reader, reader.uint32());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromPartial(object: DeepPartial<JobResultData>): JobResultData {
    ...
    message.resolvedRetryPolicy = (object.resolvedRetryPolicy !== undefined && object.resolvedRetryPolicy !== null)
      ? RetryPolicy.fromPartial(object.resolvedRetryPolicy)
      : undefined;
    return message;
  },
};
