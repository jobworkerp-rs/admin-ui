...
import { FeedDataTransport, Job, JobExecutionOverrides, JobId } from "../data/job";
...
/** Count mode for JobProcessingStatusService.CountByCondition. */
export enum CountJobProcessingStatusMode {
  /** COUNT_JOB_PROCESSING_STATUS_MODE_TOTAL - Count all rows matching the condition. */
  COUNT_JOB_PROCESSING_STATUS_MODE_TOTAL = 0,
  /** COUNT_JOB_PROCESSING_STATUS_MODE_GROUP_BY_STATUS - Count rows matching the condition grouped by processing status. */
  COUNT_JOB_PROCESSING_STATUS_MODE_GROUP_BY_STATUS = 1,
  UNRECOGNIZED = -1,
}
...
export interface JobRequest {
  ...
  using?:
    ...
    | undefined;
  /**
   * Per-job execution overrides (response_type, store_success, etc.)
...
/** Request for counting job processing statuses with the same predicates as FindByCondition. */
export interface CountJobProcessingStatusRequest {
  /** Status filter */
  status?:
    | JobProcessingStatus
    | undefined;
  ...
  minElapsedTimeMs?:
    | string
    | undefined;
  /** Count mode. Default is TOTAL. */
  mode: CountJobProcessingStatusMode;
}

export interface JobProcessingStatusCount {
  status: JobProcessingStatus;
  count: string;
}

export interface CountJobProcessingStatusResponse {
  total: string;
  counts: JobProcessingStatusCount[];
  mode: CountJobProcessingStatusMode;
}
...
export interface PurgeStaleJobsRequest {
  ...
  /**
   * When true, only purge stale RDB index rows whose job is no longer
   * referenced by either the live JobProcessingStatusRepository (Redis/Memory)
   * or the `job` table. The live status repository is the source of truth for
   * normal jobs; the `job` table is the SoT for queue types that don't keep a
   * live status entry (DbOnly, periodic, future run_after). A row is orphan
   * only when both sources have let go of the job_id, so still-queued or
   * long-running jobs are preserved.
   *
   * When false (default), purge all stale records in bulk regardless of
   * source-of-truth state.
   */
  orphanedOnly?: boolean | undefined;
}

/** Response for PurgeStaleJobs */
export interface PurgeStaleJobsResponse {
  /** Number of records marked as deleted */
  markedCount: string;
  /** Cutoff timestamp used for stale detection (milliseconds since epoch) */
  cutoffTime: string;
  /** Human-readable message */
  message: string;
}
...
export const JobRequest: MessageFns<JobRequest> = {
  encode(message: JobRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.overrides !== undefined) {
      JobExecutionOverrides.encode(message.overrides, writer.uint32(74).fork()).join();
    }
    return writer;
  },
...
function createBaseCountJobProcessingStatusRequest(): CountJobProcessingStatusRequest {
  return { status: undefined, workerId: undefined, channel: undefined, minElapsedTimeMs: undefined, mode: 0 };
}

export const CountJobProcessingStatusRequest: MessageFns<CountJobProcessingStatusRequest> = {
  encode(message: CountJobProcessingStatusRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.status !== undefined) {
      writer.uint32(8).int32(message.status);
    }
    if (message.workerId !== undefined) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): CountJobProcessingStatusRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCountJobProcessingStatusRequest();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  create(base?: DeepPartial<CountJobProcessingStatusRequest>): CountJobProcessingStatusRequest {
    return CountJobProcessingStatusRequest.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<CountJobProcessingStatusRequest>): CountJobProcessingStatusRequest {
    const message = createBaseCountJobProcessingStatusRequest();
    message.status = object.status ?? undefined;
    message.workerId = object.workerId ?? undefined;
    message.channel = object.channel ?? undefined;
...
function createBaseJobProcessingStatusCount(): JobProcessingStatusCount {
  return { status: 0, count: "0" };
}

export const JobProcessingStatusCount: MessageFns<JobProcessingStatusCount> = {
  encode(message: JobProcessingStatusCount, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.status !== 0) {
      writer.uint32(8).int32(message.status);
    }
    if (message.count !== "0") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): JobProcessingStatusCount {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseJobProcessingStatusCount();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  create(base?: DeepPartial<JobProcessingStatusCount>): JobProcessingStatusCount {
    return JobProcessingStatusCount.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<JobProcessingStatusCount>): JobProcessingStatusCount {
    const message = createBaseJobProcessingStatusCount();
    message.status = object.status ?? 0;
...
function createBaseCountJobProcessingStatusResponse(): CountJobProcessingStatusResponse {
  return { total: "0", counts: [], mode: 0 };
}

export const CountJobProcessingStatusResponse: MessageFns<CountJobProcessingStatusResponse> = {
  encode(message: CountJobProcessingStatusResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.total !== "0") {
      writer.uint32(8).int64(message.total);
    }
    for (const v of message.counts) {
      JobProcessingStatusCount.encode(v!, writer.uint32(18).fork()).join();
    }
    if (message.mode !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): CountJobProcessingStatusResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCountJobProcessingStatusResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
        ...
        case 2: {
          ...
          message.counts.push(JobProcessingStatusCount.decode(reader, reader.uint32()));
          continue;
        }
        case 3: {
          if (tag !== 24) {
  ...
  create(base?: DeepPartial<CountJobProcessingStatusResponse>): CountJobProcessingStatusResponse {
    return CountJobProcessingStatusResponse.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<CountJobProcessingStatusResponse>): CountJobProcessingStatusResponse {
    const message = createBaseCountJobProcessingStatusResponse();
    message.total = object.total ?? "0";
    message.counts = object.counts?.map((e) => JobProcessingStatusCount.fromPartial(e)) || [];
    message.mode = object.mode ?? 0;
    return message;
  },
};
...
function createBasePurgeStaleJobsRequest(): PurgeStaleJobsRequest {
  return { staleThresholdHours: "0", orphanedOnly: undefined };
}

export const PurgeStaleJobsRequest: MessageFns<PurgeStaleJobsRequest> = {
  encode(message: PurgeStaleJobsRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.staleThresholdHours !== "0") {
      writer.uint32(8).uint64(message.staleThresholdHours);
    }
    if (message.orphanedOnly !== undefined) {
      writer.uint32(16).bool(message.orphanedOnly);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PurgeStaleJobsRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePurgeStaleJobsRequest();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          ...
          message.staleThresholdHours = reader.uint64().toString();
          continue;
        }
        case 2: {
          if (tag !== 16) {
          ...
          message.orphanedOnly = reader.bool();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  create(base?: DeepPartial<PurgeStaleJobsRequest>): PurgeStaleJobsRequest {
    return PurgeStaleJobsRequest.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<PurgeStaleJobsRequest>): PurgeStaleJobsRequest {
    const message = createBasePurgeStaleJobsRequest();
    message.staleThresholdHours = object.staleThresholdHours ?? "0";
    message.orphanedOnly = object.orphanedOnly ?? undefined;
    return message;
  },
};

function createBasePurgeStaleJobsResponse(): PurgeStaleJobsResponse {
  return { markedCount: "0", cutoffTime: "0", message: "" };
}

export const PurgeStaleJobsResponse: MessageFns<PurgeStaleJobsResponse> = {
  encode(message: PurgeStaleJobsResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.markedCount !== "0") {
      writer.uint32(8).uint64(message.markedCount);
    }
    if (message.cutoffTime !== "0") {
      writer.uint32(16).int64(message.cutoffTime);
    }
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PurgeStaleJobsResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePurgeStaleJobsResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          ...
          message.markedCount = reader.uint64().toString();
          continue;
        }
        case 2: {
          if (tag !== 16) {
  ...
  create(base?: DeepPartial<PurgeStaleJobsResponse>): PurgeStaleJobsResponse {
    return PurgeStaleJobsResponse.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<PurgeStaleJobsResponse>): PurgeStaleJobsResponse {
    const message = createBasePurgeStaleJobsResponse();
...
export const JobServiceDefinition = {
  ...
  methods: {
    enqueue: {
      name: "Enqueue",
      requestType: JobRequest as typeof JobRequest,
      requestStream: false,
      responseType: CreateJobResponse as typeof CreateJobResponse,
    ...
    enqueueForStream: {
      ...
      requestType: JobRequest as typeof JobRequest,
      requestStream: false,
      responseType: ResultOutputItem as typeof ResultOutputItem,
      responseStream: true,
      options: {},
    },
    /**
    ...
    delete: {
      name: "Delete",
      requestType: JobId as typeof JobId,
      requestStream: false,
      responseType: SuccessResponse as typeof SuccessResponse,
      responseStream: false,
      options: {},
    },
    ...
    find: {
      ...
      requestType: JobId as typeof JobId,
      requestStream: false,
      responseType: OptionalJobResponse as typeof OptionalJobResponse,
      responseStream: false,
      options: {},
    },
    /** only use db or redis+db queue type, run after or periodic worker jobs */
    findList: {
      name: "FindList",
      requestType: FindListRequest as typeof FindListRequest,
      requestStream: false,
    ...
    findQueueList: {
      name: "FindQueueList",
      requestType: FindQueueListRequest as typeof FindQueueListRequest,
      requestStream: false,
      responseType: JobAndStatus as typeof JobAndStatus,
      responseStream: true,
      options: {},
    },
    ...
    findListWithProcessingStatus: {
      ...
      requestType: FindListWithProcessingStatusRequest as typeof FindListWithProcessingStatusRequest,
      requestStream: false,
      responseType: JobAndStatus as typeof JobAndStatus,
      responseStream: true,
      options: {},
    ...
    count: {
      name: "Count",
      requestType: CountCondition as typeof CountCondition,
      requestStream: false,
...
export const JobProcessingStatusServiceDefinition = {
  ...
  methods: {
    find: {
      ...
      requestType: JobId as typeof JobId,
      requestStream: false,
      responseType: OptionalJobProcessingStatusResponse as typeof OptionalJobProcessingStatusResponse,
      responseStream: false,
      options: {},
    },
    findAll: {
      ...
      requestStream: false,
      responseType: JobProcessingStatusResponse as typeof JobProcessingStatusResponse,
      responseStream: true,
      options: {},
    },
    /**
    ...
    findByCondition: {
      ...
      requestType: FindJobProcessingStatusRequest as typeof FindJobProcessingStatusRequest,
      requestStream: false,
      responseType: JobProcessingStatusDetailResponse as typeof JobProcessingStatusDetailResponse,
      responseStream: true,
      options: {},
    },
    /**
     * Count active rows using the same RDB index predicates as FindByCondition.
     * Returns UNIMPLEMENTED error if JOB_STATUS_RDB_INDEXING=false
     */
    countByCondition: {
      name: "CountByCondition",
      requestType: CountJobProcessingStatusRequest as typeof CountJobProcessingStatusRequest,
      requestStream: false,
      responseType: CountJobProcessingStatusResponse as typeof CountJobProcessingStatusResponse,
      responseStream: false,
      options: {},
    },
    /**
    ...
    cleanup: {
      name: "Cleanup",
      requestType: CleanupRequest as typeof CleanupRequest,
      requestStream: false,
      responseType: CleanupResponse as typeof CleanupResponse,
      responseStream: false,
      options: {},
    },
    /**
     * Purge stale job processing status records
     *
     * Marks stale records (deleted_at IS NULL, updated_at older than threshold) as logically deleted.
     * Records whose job has a future run_after_time are excluded.
     *
     * Modes:
     * - orphaned_only=false (default): Purges all stale records in bulk.
     * - orphaned_only=true: Purges stale records whose job_id is absent from BOTH the live JobProcessingStatusRepository (Redis/Memory) AND the `job` table.
     *
     * Redis/Memory is the source of truth for live job processing state; the
     * `job` table is the SoT for queue types without a live-status entry
     * (DbOnly, periodic, future run_after). The RDB `job_processing_status`
     * table is an eventually-consistent search view that may lag. The
     * orphaned_only=true mode is intended to sweep leftover index rows when
     * `cleanup_job`'s best-effort `mark_deleted_by_job_id` write failed
     * (terminal state, no self-healing).
     *
     * Requirements:
     * - JOB_STATUS_RDB_INDEXING=true (returns FAILED_PRECONDITION if disabled)
     * - AUTH_TOKEN environment variable must match jobworkerp-auth header (if set)
    ...
    purgeStaleJobs: {
      name: "PurgeStaleJobs",
      requestType: PurgeStaleJobsRequest as typeof PurgeStaleJobsRequest,
      requestStream: false,
      responseType: PurgeStaleJobsResponse as typeof PurgeStaleJobsResponse,
      responseStream: false,
      options: {},
    },
  },
...
export const JobRestoreServiceDefinition = {
  ...
  methods: {
    restore: {
      name: "Restore",
      requestType: JobRestoreRequest as typeof JobRestoreRequest,
      requestStream: false,
      responseType: SuccessResponse as typeof SuccessResponse,
      responseStream: false,
      options: {},
