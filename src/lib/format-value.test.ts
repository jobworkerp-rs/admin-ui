...
import { formatDecodedValue, formatProtoBytes } from './format-value';

const encode = (s: string) => new TextEncoder().encode(s);

describe('formatDecodedValue', () => {
    it('pretty-prints a JSON object string', () => {
        expect(formatDecodedValue('{"a":1,"b":2}')).toBe('{\n  "a": 1,\n  "b": 2\n}');
    });

    it('pretty-prints a JSON array string', () => {
        expect(formatDecodedValue('[1,2]')).toBe('[\n  1,\n  2\n]');
    });

    it('keeps a plain string unchanged (with real newlines)', () => {
        expect(formatDecodedValue('line1\nline2')).toBe('line1\nline2');
    });

    it('does NOT unwrap a numeric string', () => {
        expect(formatDecodedValue('3000')).toBe('3000');
    });

    it('returns invalid-JSON-looking string as-is', () => {
        expect(formatDecodedValue('{not json')).toBe('{not json');
    });

    it('pretty-prints a real object value', () => {
        expect(formatDecodedValue({ a: 1 })).toBe('{\n  "a": 1\n}');
    });
    ...
    it('unwraps a nested JSON string inside an object value', () => {
        expect(formatDecodedValue({ config: '{"a":1}' })).toBe(
            '{\n  "config": {\n    "a": 1\n  }\n}',
        );
    });
    ...
    it('unwraps nested JSON strings inside arrays', () => {
        expect(formatDecodedValue(['{"a":1}', 'plain'])).toBe(
            '[\n  {\n    "a": 1\n  },\n  "plain"\n]',
        );
    });

    it('keeps plain nested strings untouched while unwrapping JSON ones', () => {
        expect(formatDecodedValue({ note: 'line1\nline2', cfg: '[1,2]' })).toBe(
            '{\n  "note": "line1\\nline2",\n  "cfg": [\n    1,\n    2\n  ]\n}',
        );
    });

    it('returns empty string for null/undefined', () => {
        expect(formatDecodedValue(null)).toBe('');
        expect(formatDecodedValue(undefined)).toBe('');
    });

    it('stringifies primitives', () => {
        expect(formatDecodedValue(42)).toBe('42');
        expect(formatDecodedValue(true)).toBe('true');
    });

    it('handles cyclic references without overflowing the stack', () => {
        const obj: Record<string, unknown> = { a: 1 };
        obj.self = obj;
        expect(formatDecodedValue(obj)).toBe(
            '{\n  "a": 1,\n  "self": "[Circular]"\n}',
        );
    });
    ...
    it('handles deeply nested JSON strings without throwing', () => {
        ...
        expect(formatDecodedValue(deep)).toBe(
            '{\n  "a": {\n    "b": {\n      "c": 1\n    }\n  }\n}',
        );
    });
});
